...
def comparer_questions_rapport(questions, embeddings_rapport, sections_rapport, titles_rapport, llm_chain, embed_model, top_k=3):
    ...
    with ThreadPoolExecutor(max_workers=12) as executor:
        ...
        for _, row in tqdm(questions.iterrows(), total=len(questions), desc="Comparing questions"):
            ...
            # Ajouter la colonne 'paragraph' ici pour l'associer à la question
            paragraph = row['paragraph']
            futures.append(executor.submit(trouver_sections_et_generer_reponse, question, paragraph,
                           embeddings_rapport, sections_rapport, titles_rapport, llm_chain, embed_model, top_k))
        # Récupérer les résultats
        for future in tqdm(futures, desc="Retrieving answers"):
            try:
                question, paragraph, retrieved_sections, generated_answer = future.result()
                results.append({
                    "paragraph": paragraph,  # Inclure le paragraphe dans les résultats
                    "question": question,
...
def trouver_sections_et_generer_reponse(question, paragraph, embeddings_rapport, sections_rapport, titles_rapport, llm_chain, embed_model, top_k):
    ...
    return question, paragraph, " ".join(top_k_sections), generated_answer
